...
void COverlappedWindow::countWords() {


	DWORD textLen = SendMessage(hwndEdit, WM_GETTEXTLENGTH, 0, 0);

	DWORD bufferLen = (textLen + 1) * sizeof(wchar_t);
	...
	hinstLib = LoadLibrary(TEXT("NotepadDLL.dll"));
	...
	if (hinstLib != NULL)
	{
		...
		if (NULL != WordsCount)
		{
			...
			int wordCount = (WordsCount)(buffer);
			std::wostringstream stros;
			stros << wordCount;
			MessageBox(handle, stros.str().c_str(), L"Notepad", MB_OK);
		}
